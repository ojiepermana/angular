...
Local schematic that generates a **lightweight Angular SDK** from any OpenAPI 3.x spec
(including 3.2.0). Output: typed `HttpClient` services, tree-shakeable fn modules,
optional metadata (permissions / validators), and a navigation tree.

## Quick start

```bash
# 1. Build the generator (once, and after each source change)
bun run gen:sdk:build

# 2. Scaffold a config file at the workspace root
bun run gen:sdk:init

# 3. Edit sdk.config.json (spec path, output path, mode, features)
...
The package exposes two schematics via `collection.json`:
...
| `navigation` | `navigation.ts` — nested tree built from tag `x-parent` / `parent` annotations. |
